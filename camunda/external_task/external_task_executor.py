...
class ExternalTaskExecutor:
    ...
    async def execute_task(self, task, action, loop):
    ...
    def _handle_task_success(self, task_id, task_result, topic):
        ...
        if self.external_task_client.complete(
    ...
    def _handle_task_failure(self, task_id, task_result, topic):
        ...
        if self.external_task_client.failure(
            task_id,
            ...
            self._log_with_context(
                f"Marked task failed - Topic: {topic} task_result: {task_result}",
                task_id,
            )
        else:
            self._log_with_context(
