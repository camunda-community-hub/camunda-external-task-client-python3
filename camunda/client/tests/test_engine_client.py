...
class EngineClientTest(IsolatedAsyncioTestCase):
    ...
    @aioresponses()
    async def test_start_process_success(self, mocked: aioresponses):
        resp_payload = {
            "links": [
                {
                    "method": "GET",
        ...
        mocked.post(self.client.get_start_process_instance_url(self.process_key, self.tenant_id),
                    payload=resp_payload, status=HTTPStatus.OK)
    ...
    @aioresponses()
    async def test_start_process_not_found_raises_exception(self, mocked):
        resp_payload = {
            "type": "RestException",
    ...
    @aioresponses()
    async def test_start_process_bad_request_raises_exception(self, mocked):
        client = EngineClient()
    ...
    @aioresponses()
    async def test_start_process_server_error_raises_exception(self, mocked: aioresponses):
    ...
    @aioresponses()
    async def test_get_process_instance_success(self, mocked: aioresponses):
        resp_payload = [
            {
                "links": [],
    ...
    @aioresponses()
    async def test_get_process_instance_bad_request_raises_exception(self, mocked: aioresponses):
    ...
    @aioresponses()
    async def test_get_process_instance_server_error_raises_exception(self, mocked: aioresponses):
        get_process_instance_url = f"{ENGINE_LOCAL_BASE_URL}/process-instance" \
                                   f"?processDefinitionKey={self.process_key}" \
                                   f"&tenantIdIn={self.tenant_id}" \
                                   f"&variables=intVar_XXX_1,strVar_eq_hello"
    ...
    @aioresponses()
    async def test_correlate_message_with_only_message_name(self, mocked: aioresponses):
    ...
    @aioresponses()
    async def test_correlate_message_with_business_key(self, mocked: aioresponses):
        expected_request_payload = {
            "messageName": "CANCEL_MESSAGE",
            "withoutTenantId": True,
            "businessKey": "123456",
    ...
    @aioresponses()
    async def test_correlate_message_with_tenant_id(self, mocked: aioresponses):
        expected_request_payload = {
            "messageName": "CANCEL_MESSAGE",
    ...
    @aioresponses()
    async def test_correlate_message_invalid_message_name_raises_exception(self, mocked: aioresponses):
    ...
    @aioresponses()
    async def test_get_process_instance_variable_without_meta(self, mocked: aioresponses):
    ...
    @aioresponses()
    async def test_get_process_instance_variable_with_meta(self, mocked: aioresponses):
        process_instance_id = "c2c68785-9f42-11ea-a841-0242ac1c0004"
        variable_name = "var1"
        process_instance_var_url = \
            f"{ENGINE_LOCAL_BASE_URL}/process-instance/{process_instance_id}/variables/{variable_name}"
